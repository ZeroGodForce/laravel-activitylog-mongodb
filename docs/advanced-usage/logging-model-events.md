...
The package can automatically log events such as when a model is created, updated and deleted. To make this work all you need to do is let your model use the `ZeroGodForce\Activitylog\Traits\LogsActivity`-trait.
...
Note that we start from sensible defaults, but any of them can be overridden as needed by chaining fluent methods. Review the `ZeroGodForce\Activitylog\LogOptions` class for full list of supported options.
